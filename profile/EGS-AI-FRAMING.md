...
**Cumulative craft.** Chess strategy builds on 1,500 years of recorded analysis. Openings have been studied, named, and refined across centuries. Each generation inherited what the previous generation learned and added to it. The craft compounded. The Royal Game of Ur had its own version — a Babylonian clay tablet from 177 BCE was a teaching document for a game already two thousand years old.
...
**"Processor bodies are pseudocode / documentation."** The `.runs-prim` body notation is a draft sketch of a formal expression language, not human-readable documentation that developers reimplement in their language of choice. The design intent is a language-agnostic formal notation with unambiguous semantics — the enduring artifact that any future runtime can parse and compile. The specification of this language is an open design question (see [EXPRESSION_LANGUAGE.md](https://github.com/enduring-game-standard/runs-spec/blob/main/EXPRESSION_LANGUAGE.md)), but the design direction is clear: pure, total, deterministic.
